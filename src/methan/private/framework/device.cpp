  ...
  m_name(std::move(name))
{
  ...
  METHAN_LOG_DEBUG(context->logger, "AbstractDevice::AbstractDevice(\"{}\", {}) registered", m_name, m_descriptor.uuid);
  context->devices.push_back(this);
}

METHAN_API Methan::AbstractDevice::~AbstractDevice()
{
  METHAN_LOG_DEBUG(context()->logger, "AbstractDevice::AbstractDevice(\"{}\", {}) destroyed", name(), m_descriptor.uuid);
