...
TEST_CASE("TensorShape test", "[tensor]")
{
...
#ifdef METHAN_EXPAND_ASSERTION
    REQUIRE_THROWS_AS(sh.offset_of({3, 0, 0}), Methan::Exception);
    REQUIRE_THROWS_AS(sh.offset_of({2, 1, 0}), Methan::Exception);
    REQUIRE_THROWS_AS(sh.offset_of({0, 0, 2}), Methan::Exception);
#endif

    REQUIRE(sh.offset_of({0, 0, 0}) == shape.offset_of({ 2, 1, 0 }));
    REQUIRE(sh.offset_of({1, 0, 1}) == shape.offset_of({ 3, 1, 1 }));
    REQUIRE(sh.offset_of({2, 0, 1}) == shape.offset_of({ 4, 1, 1 }));
    REQUIRE(sh.indices_of(7) == std::vector<uint32_t>{0, 0, 0});
    ...
    for(size_t i = 0; i < shape.size(); ++i)
    {
        ...
        if(vec[1] != 1 || vec[0] < 2) {
#ifdef METHAN_EXPAND_ASSERTION
            REQUIRE_THROWS_AS(sh.indices_of(i), Methan::Exception);
#endif
        }
        else {
            REQUIRE(sh.offset_of(sh.indices_of(i)) == i);
        }
    }
}